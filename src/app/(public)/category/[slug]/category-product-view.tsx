...
const CategoryProductsView = ({ searchParams, slug }: Props) => {
  const {
    brands,
    setBrands,
    indications,
  ...
  const { data, fetchNextPage, hasNextPage, isFetching, isFetchingNextPage } =
    useInfiniteQuery({
      ...
      queryFn: async ({ pageParam }) => {
        const { getProductsOfCategory } = await import("@/app/api/category");

        const { data, error } = await getProductsOfCategory(slug, {
          from: pageParam,
          size: 12,
          brands: brands,
